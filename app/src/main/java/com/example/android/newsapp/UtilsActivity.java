...
public class UtilsActivity {
    ...
    /**
     * Query the USGS dataset and return an {@link Article} object to represent a single earthquake.
     */
    public static ArrayList<Article> fetchArticleData(String requestUrl) {
        ...
        List<Article> earthquakes = extractArticles(jsonResponse);

        // Return the {@link Event}
        return (ArrayList<Article>) earthquakes;
    }
    ...
    public static ArrayList<Article> extractArticles(String jsonResponse) {
        ...
        ArrayList<Article> earthquakes = new ArrayList<>();
        ...
        try {
            ...
            JSONObject response = (JSONObject) rootObject.getJSONObject("response");
            JSONArray articleArray = (JSONArray) response.optJSONArray("results");

            for (int i = 0; i < articleArray.length(); i++) {
                JSONObject article = (JSONObject) articleArray.get(i);
                JSONObject field = (JSONObject) article.getJSONObject("fields");
                JSONArray tags = (JSONArray) article.getJSONArray("tags");
                JSONObject articleContributor= (JSONObject) tags.getJSONObject(0);
                ...
                earthquakes.add(new Article(sectionName, articleTitle, url, imageURL, contributor, time));
            }
